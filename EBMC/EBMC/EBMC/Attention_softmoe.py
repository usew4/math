...
class Attention(nn.Module):
    ...
    def forward(self, x, mask_modality, mask=None):
        ...
        if mask is not None:
            ...
            # All-padding rows must not produce NaNs, including in backward.
            scores = attn.masked_fill(~mask[:, None, None, :], torch.finfo(attn.dtype).min)
            probs = scores.softmax(dim=-1) * mask[:, None, None, :]
            probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
            attn = self.attn_drop(probs.type_as(x))
