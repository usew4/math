...
class Q2ReliabilityModel(nn.Module):
    def __init__(self, config: dict, audio_mean: torch.Tensor, audio_std: torch.Tensor,
        ...
        self.text_only = bool(options.get("text_only", False))
        if self.text_only and any(options.get(key, False) for key in (
                "word_aligned_mag", "unimodal_auxiliary_heads", "decision_fusion",
                "neutral_auxiliary_head", "shared_query_sequence_fusion")):
            raise ValueError("text_only is an independent text baseline")
        use_unimodal_heads = bool(options.get("unimodal_auxiliary_heads", False))
        if use_unimodal_heads and any(options.get(key, False) for key in (
                "word_aligned_mag", "audio_text_guided", "neutral_auxiliary_head",
                "decision_fusion", "shared_query_sequence_fusion")):
            raise ValueError("Unimodal supervision requires the independent, sentence-level baseline")
        ...
        # Only old checkpoints with this saved flag use the discontinued pool.
        if options.get("text_attention_pool", False):
            from .q2_legacy_text_pool import MaskedTextAttentionPool
            with torch.random.fork_rng():
                torch.manual_seed(2042)
                self.text_attention_pool = MaskedTextAttentionPool()
            self.text_attention_mix_logit = nn.Parameter(torch.tensor(-2.0))
        else:
            self.text_attention_pool = None
        if options.get("shared_query_sequence_fusion", False):
            # Compatibility only: active MAG configs do not use this branch.
            from .q2_legacy_videofusion import SharedQuerySequenceFusion
            with torch.random.fork_rng():
                torch.manual_seed(2042)
                self.shared_query_fusion = SharedQuerySequenceFusion(dim, dropout)
        else:
            self.shared_query_fusion = None
        # Keep common weights and the global RNG identical to the baseline.
        if use_unimodal_heads:
            with torch.random.fork_rng():
                torch.manual_seed(3042)
                self.unimodal_heads = nn.ModuleDict({
                    name: nn.Linear(dim, 3) for name in ("text", "audio", "vision")})
        else:
            self.unimodal_heads = None
    ...
    def forward(self, vision: torch.Tensor, audio: torch.Tensor,
        ...
        text_summary = text_mean
        if self.text_attention_pool is not None:
            attentive_summary, _ = self.text_attention_pool(token_states, token_mask)
            mix = torch.sigmoid(self.text_attention_mix_logit)
            text_summary = (1.0 - mix) * text_mean + mix * attentive_summary
        text_feature = self.text_projection(torch.cat((token_states[:, 0], text_summary), dim=-1))

        if self.text_only:
            fused = self.dropout(self.fusion_norm(text_feature))
        ...
        fusion_input = text_feature + audio_gate * audio_feature + vision_gate * vision_feature
        if self.shared_query_fusion is not None:
            fusion_input = fusion_input + self.shared_query_fusion(
                token_states, token_mask, audio_values, vision_values,
                audio_mask, vision_mask)
        fused = self.dropout(self.fusion_norm(fusion_input))
        ...
        if self.unimodal_heads is not None:
            # Whole-sample absence is masked out of auxiliary supervision/metrics.
            auxiliary = {
                "text": (self.unimodal_heads["text"](text_feature), token_mask.sum(dim=1) > 2),
                "audio": (self.unimodal_heads["audio"](audio_feature), audio_mask.any(dim=1)),
                "vision": (self.unimodal_heads["vision"](vision_feature), vision_mask.any(dim=1)),
            }
            return logits, score, auxiliary
        if self.decision_fusion:
