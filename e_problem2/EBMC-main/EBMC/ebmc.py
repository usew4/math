...
class EBMC(nn.Module):
    ...
    def __init__(self, args, adim, tdim, vdim, D_e, n_classes,
        ...
        if getattr(args, 'q2_classification', False):
            self.nlp_reg_head = nn.Linear(D, 1)
            self.availability_proj = nn.Linear(3, D, bias=False)
    ...
    def _compute_cce(self, z_a_c, z_t_c, z_v_c, z_a_s, z_t_s, z_v_s,
                     x_a, x_t, x_v, do_cf, availability=None):
        ...
        return compute_cce_loss(
            ...
            z_a_s, z_t_s, z_v_s,
            x_a, x_t, x_v,
            availability=availability if getattr(self.args, 'q2_classification', False) else None
        )
    ...
    def _load_teacher(self, inputfeats, input_features_mask, umask):
        ...
        if not hasattr(self, 'teacher_model'):
            teacher = build_model(self.args, self.adim, self.tdim, self.vdim)
            stage1_path = getattr(self.args, 'teacher_path', None) or os.path.join(
                config.MODEL_DIR, 'stage_1',
                f"{self.args.dataset}_{self.args.test_condition}_stage1_best.pth"
            )
            teacher.load_state_dict(torch.load(stage1_path, map_location=self.device,
                                               weights_only=True))
            teacher.to(self.device).eval().requires_grad_(False)
            # Keep the frozen teacher out of the student's state_dict and optimizer.
            object.__setattr__(self, 'teacher_model', teacher)

        with torch.no_grad():
            _, _, t_logits_a, t_logits_t, t_logits_v, _, _ = self.teacher_model(
                inputfeats, input_features_mask, umask, do_cf=False
            )
    ...
    def forward(self, inputfeats, input_features_mask=None, umask=None,
        ...
        if getattr(self, 'frame_seq', False):
            ...
            if availability is None:
                availability = torch.stack([ma.any(1), mt.any(1), mv.any(1)], dim=1)
        ...
        if first_stage:
            ...
            return self._forward_stage2(
                ...
                inputfeats, input_features_mask, availability
            )

    def _forward_stage1(self, x_a, x_t, x_v, B, seq_len, label, umask,
                        do_cf, weight_save, availability=None):
        ...
        cfd_loss = self._compute_cce(
            ...
            x_a, x_t, x_v, do_cf, availability
        )
    ...
    def _forward_stage2(self, x_a, x_t, x_v,
                        weight_a, weight_t, weight_v,
                        B, seq_len, label, umask, do_cf, weight_save,
                        inputfeats, input_features_mask, availability=None):
        ...
        if getattr(self.args, 'q2_classification', False) and availability is not None:
            x_out_a = x_out_a * availability[:, 0, None, None]
            x_out_t = x_out_t * availability[:, 1, None, None]
            x_out_v = x_out_v * availability[:, 2, None, None]
        ...
        (z_a_c, z_t_c, z_v_c,
         z_a_s, z_t_s, z_v_s,
         cid_losses) = self.cid_integration(
            x_out_a, x_out_t, x_out_v, y=label, umask=umask_s,
            return_loss=True,
            availability=availability if getattr(self.args, 'q2_classification', False) else None
        )
        ...
        cfd_loss = self._compute_cce(
            z_a_c, z_t_c, z_v_c, z_a_s, z_t_s, z_v_s,
            x_out_a, x_out_t, x_out_v, do_cf, availability
        )
        ...
        hidden = u + res
        if getattr(self.args, 'q2_classification', False) and availability is not None:
            hidden = hidden + self.availability_proj(availability.float()).unsqueeze(1)
        logits_c = self.nlp_head(hidden)

        emc_loss = torch.zeros((), device=self.device)
        imtd_loss = torch.zeros((), device=self.device)
        if label is not None:
            # The frozen Stage-I teacher is needed only for training losses.
