...
def compute_emc_loss(x_out_a, x_out_t, x_out_v,
    ...
    if availability is not None:
        complete = availability.all(dim=1)
        if not complete.any():
            return x_out_a.sum() * 0
        x_out_a, x_out_t, x_out_v = (x[complete] for x in
                                      (x_out_a, x_out_t, x_out_v))
        logits_a, logits_t, logits_v = (x[complete] for x in
                                         (logits_a, logits_t, logits_v))
        teacher_logits_a, teacher_logits_t, teacher_logits_v = (
            x[complete] if x is not None else None
            for x in (teacher_logits_a, teacher_logits_t, teacher_logits_v))
        label, umask = label[complete], umask[complete]
    ...
    is_classification = (args.dataset in ['IEMOCAPFour', 'IEMOCAPSix']
                         or getattr(args, 'q2_classification', False))
