...
def compute_cce_loss(cf_generator, pred_heads,
    ...
    if availability is not None:
        # A counterfactual requires both its source and target modalities.
        pairs = ((cf_a, x_a, pred_heads['a'], availability[:, 0] & availability[:, 1]),
                 (cf_t, x_t, pred_heads['t'], availability[:, 0] & availability[:, 1]),
                 (cf_v, x_v, pred_heads['v'], availability[:, 1] & availability[:, 2]))
        terms = []
        for counterfactual, target, head, observed in pairs:
            if not observed.any():
                continue
            reconstruction = (counterfactual - target.detach()).square().mean(dim=(1, 2))
            consistency = (head(target) - head(counterfactual)).abs().mean(dim=(1, 2))
            terms.append(((reconstruction + consistency) * observed).sum()
                         / observed.sum().clamp_min(1))
        return torch.stack(terms).mean() if terms else cf_a.sum() * 0
